...
import { Mic, MicOff, Loader2, Volume2, VolumeX, Keyboard, Send, Trash2, MessageSquare, X, Download } from "lucide-react";
...
export default function App() {
  ...
  useEffect(() => {
    scrollToBottom();
  }, [messages, appState, showHistoryDrawer]);
  ...
  return (
    <div className="h-[100dvh] w-screen bg-[#050505] text-white flex flex-col items-center justify-between font-sans relative overflow-hidden m-0 p-0">
      ...
      <header className="absolute top-0 left-0 w-full flex justify-between items-center z-20 shrink-0 px-6 py-4 md:px-12 md:py-6">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2 rounded-full bg-white/5 hover:bg-violet-500/20 hover:text-violet-400 transition-colors border border-white/10 relative"
            title="Download Source Code (ZIP)"
          >
            <Download size={18} className="opacity-70 animate-pulse" />
          </button>
          <button
          ...
          >
            ...
            {messages.length > 0 && (
              <span className="absolute -top-1 -right-1 min-w-[16px] h-4 bg-violet-500 rounded-full text-[9px] flex items-center justify-center font-bold px-1 select-none">
                {messages.length}
              </span>
            )}
          </button>
      ...
      {/* Subtitles Overlay */}
      <div className="absolute bottom-28 md:bottom-32 left-1/2 -translate-x-1/2 w-full max-w-xl px-6 text-center z-20 pointer-events-none">
        <AnimatePresence mode="wait">
          {appState !== "idle" && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -10 }}
              className="bg-black/60 backdrop-blur-md px-6 py-3 rounded-2xl border border-white/10 shadow-2xl flex flex-col gap-1 items-center"
            >
              <div className="text-[10px] uppercase tracking-[0.2em] font-mono opacity-50 text-violet-400">
                {appState === "listening" && "আমি শুনছি... (Listening)"}
                {appState === "processing" && "একটু ভাবছি... (Thinking)"}
                {appState === "speaking" && "জোয়া কথা বলছে... (Speaking)"}
              </div>
              <div className="text-sm md:text-base font-medium text-white/90 leading-relaxed font-sans">
                {appState === "speaking" && (messages.filter(m => m.sender === "zoya").pop()?.text || "উত্তর রেডি হচ্ছে...")}
                {appState === "listening" && (messages.filter(m => m.sender === "user").pop()?.text || "বলো, আমি মনোযোগ দিয়ে শুনছি...")}
                {appState === "processing" && "একটি ঝাল-মিষ্টি উত্তর সাজাচ্ছি..."}
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {showHistoryDrawer && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-0 bg-black/60 backdrop-blur-sm z-30 pointer-events-auto cursor-pointer"
            />
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 h-full w-full max-w-md bg-[#080d12]/95 border-l border-white/10 backdrop-blur-md shadow-2xl z-40 flex flex-col pointer-events-auto"
            >
              {/* Drawer Header */}
              <div className="flex justify-between items-center px-6 py-5 border-b border-white/10 bg-white/[0.02]">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-serif font-medium tracking-wide text-white/90">Conversation Log</h3>
                </div>
                <button
                  onClick={() => setShowHistoryDrawer(false)}
              ...
              <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4 scrollbar-hide">
                {messages.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center opacity-40 py-20">
                    <MessageSquare size={36} className="mb-3 stroke-1 text-violet-400" />
                    <p className="text-sm font-mono">No messages yet.</p>
                    <p className="text-xs max-w-[200px] mt-1 font-sans">Start speaking or type a message to chat with Zoya.</p>
                  </div>
                ) : (
                  messages.map((msg, index) => (
                    <motion.div
                      ...
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: Math.min(0.1, index * 0.02) }}
                      className={`flex flex-col max-w-[85%] ${msg.sender === "user" ? "ml-auto items-end" : "mr-auto items-start"}`}
                    >
                      <span className="text-[10px] font-mono opacity-40 uppercase mb-1">
                        {msg.sender === "user" ? "Ashwani (You)" : "Zoya"}
                      </span>
                      <div
                        className={`px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                          msg.sender === "user"
                            ? "bg-violet-600/20 text-violet-100 border border-violet-500/30 rounded-tr-none"
                            : "bg-pink-600/15 text-pink-100 border border-pink-500/25 rounded-tl-none"
                        }`}
                      >
                        {msg.text}
                      </div>
              ...
              {/* Drawer Footer */}
              <div className="p-4 border-t border-white/10 bg-white/[0.01] flex justify-between items-center text-xs font-mono text-white/40">
                <span>Total Messages: {messages.length}</span>
                {messages.length > 0 && (
                  <button
                    onClick={() => {
                      if (confirm("Clear all conversational records?")) {
                        setMessages([]);
                        resetZoyaSession();
                      }
                    }}
                    className="text-red-400 hover:text-red-300 hover:underline transition-colors flex items-center gap-1 cursor-pointer"
                  >
